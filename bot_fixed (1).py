...
async def eumetsat_capabilities():
    params = {"service":"WMS","request":"GetCapabilities","version":"1.3.0"}
    async with aiohttp.ClientSession() as session:
        async with session.get(EUMETSAT_WMS, params=params, timeout=25) as resp:
            resp.raise_for_status()
            return await resp.text()

def eumetsat_find_layer(xml_text, preferred):
    # Keep this deliberately tolerant: EUMETSAT changes layer names between
    # collections. Prefer requested names, then any Meteosat/SEVIRI layer.
    import re
    names = re.findall(r"<(?:\w+:)?Name>([^<]+)</(?:\w+:)?Name>", xml_text)
    ...
    for needle in preferred:
        ...
        for n, ln in low:
            if needle in ln:
                return n
    for n, ln in low:
        if "geocolour" in ln and ("rgb" in ln or "colour" in ln):
            return n
    for n, ln in low:
        if "10.5" in ln or "ir108" in ln or "infrared" in ln or "ir" in ln:
            return n
    for n, ln in low:
        if "seviri" in ln or "meteosat" in ln:
            return n
    return None

async def eumetsat_wms_png(layer, bbox, width=1000, height=700):
    params = {
        "service":"WMS","request":"GetMap","version":"1.3.0",
        "layers":layer,"styles":"","format":"image/png",
        "transparent":"false","crs":"EPSG:4326",
        "bbox":",".join(f"{v:.5f}" for v in bbox),
        "width":str(width),"height":str(height)
    }
    async with aiohttp.ClientSession() as session:
        async with session.get(EUMETSAT_WMS, params=params, timeout=40) as resp:
            raw = await resp.read()
            ctype = (resp.headers.get("Content-Type") or "").lower()
            if resp.status != 200 or not raw.startswith(b"\x89PNG"):
                raise RuntimeError(f"EUMETSAT WMS не вернул PNG ({resp.status}, {ctype})")
            return raw

def make_satellite_image(city, lat, lon, raw_png, product_name):
    img = Image.open(io.BytesIO(raw_png)).convert("RGB")
    draw = ImageDraw.Draw(img)
    w,h=img.size
    # Geographic marker is approximate in the requested bounding box.
    margin=0.0
    x = w/2
    y = h/2
    r=8
    ...
    draw.rounded_rectangle((12,12,300,62), radius=10, fill=(0,0,0,165))
    draw.text((24,22), f"{city} • {product_name}", fill="white")
    out=io.BytesIO()
...
async def send_satellite(message, city):
    ...
    if lat is None:
        ...
        return
    try:
        # Regional view around the city.
        dlat, dlon = 12.0, 18.0
        bbox=(max(-89,lat-dlat), max(-180,lon-dlon), min(89,lat+dlat), min(180,lon+dlon))
        caps=await eumetsat_capabilities()
        rgb=eumetsat_find_layer(caps, [
            "geocolour","geo_colour","natural_color","naturalcolour",
            "rgb","cloud_rgb","clouds_rgb"
        ])
        ir=eumetsat_find_layer(caps, [
            "ir_108","ir108","10.5","10_5","infrared","ir"
        ])
        candidates=[(rgb,"GeoColour RGB"),(ir,"IR 10.8 µm")]
        last=None
        for layer,label in candidates:
            if not layer:
                continue
            try:
                raw=await eumetsat_wms_png(layer,bbox)
                img=make_satellite_image(resolved,lat,lon,raw,label)
                await message.answer_photo(
                    BufferedInputFile(img.read(),filename="eumetsat_satellite.png"),
                    caption=f"🛰 <b>Спутник — {html.escape(resolved)}</b>\n{html.escape(label)} • EUMETSAT Meteosat",
                    parse_mode="HTML"
                )
                return
            except Exception as e:
                last=e
        raise RuntimeError(str(last) if last else "Не найден подходящий слой Meteosat")
    except Exception as e:
        logging.exception("satellite")
        await message.answer(f"⚠️ Не удалось загрузить спутник. {html.escape(str(e))}",parse_mode="HTML")
...
async def cmd_help(message:types.Message):
    await message.answer("Личные сообщения: 🌤 Прогноз, 📈 Графики, 🌧 Радар, 🛰 Спутник и ⚙️ Настройки.\nГруппы: /wz Самара, /graph Самара и /rad Самара.")
...
async def radar_button(message:types.Message,state:FSMContext):
    ...
    await message.answer("🏙 Введи город для радара:",reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="⬅️ Назад")]],resize_keyboard=True))

@dp.message(Form.radar_city)
async def radar_city_handler(message:types.Message,state:FSMContext):
    await state.clear()
    await send_radar(message,message.text.strip())
    await message.answer("Главное меню",reply_markup=main_menu())

@dp.message(F.text=="🛰 Спутник",F.chat.type=="private")
async def satellite_button(message:types.Message,state:FSMContext):
    await state.set_state(Form.satellite_city)
    await message.answer("🏙 Введи город для спутника:",reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="⬅️ Назад")]],resize_keyboard=True))

@dp.message(Form.satellite_city)
async def satellite_city_handler(message:types.Message,state:FSMContext):
    await state.clear()
    await send_satellite(message,message.text.strip())
    await message.answer("Главное меню",reply_markup=main_menu())
