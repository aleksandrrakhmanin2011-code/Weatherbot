...
FIRMS_SOURCES = "fires_viirs_noaa21_24,fires_viirs_noaa20_24"
...
async def make_fire_image(city, lat, lon):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(wms_url, params=params, headers=headers, timeout=35) as resp:
            ...
            if resp.status != 200:
                raise RuntimeError(f"FIRMS WMS HTTP {resp.status}: {overlay_data[:160]!r}")
