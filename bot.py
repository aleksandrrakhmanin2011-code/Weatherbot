...
async def cmd_keysi(message: types.Message, state: FSMContext):
    """Open the admin authentication flow."""
    if not ADMIN_KEY:
        await message.answer("⚠️ Админ-доступ не настроен: не задан ADMIN_KEY.")
        return

    # Allow /keysi <password> as well as a two-step /keysi -> password flow.
    parts = message.text.split(maxsplit=1)
    if len(parts) == 2:
        if parts[1].strip() == ADMIN_KEY:
...
@dp.message(Form.admin_key, F.chat.type == "private")
async def cmd_keysi_password(message: types.Message, state: FSMContext):
    if not ADMIN_KEY:
        await state.clear()
        await message.answer("⚠️ Админ-доступ не настроен: не задан ADMIN_KEY.")
        return

    if (message.text or "").strip() == ADMIN_KEY:
        admin_sessions[message.from_user.id] = time.time() + ADMIN_SESSION_TTL
        await state.clear()
        await message.answer("🔐 <b>Админ-панель</b>\n\nВыбери действие:",
                             parse_mode="HTML", reply_markup=admin_menu())
