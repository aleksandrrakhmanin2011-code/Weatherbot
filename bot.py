...
async def make_fire_image(city, lat, lon):
    key=os.getenv("FIRMS_MAP_KEY") or os.getenv("NASA_FIRMS_MAP_KEY")
    if not key:
    ...
    session=await http_session()
    # 3x3 map tiles are independent: fetch them concurrently instead of 9 serial requests.
    async def tile_job(tx,ty):
        try:
            data=await _download_image(session,OSM_TILE_URL.format(z=zoom,x=tx,y=ty),headers)
            tile=Image.open(io.BytesIO(data)).convert("RGB")
            return tx,ty,tile
        except Exception:
            logging.warning("OSM tile failed z=%s x=%s y=%s",zoom,tx,ty)
            return None
    tiles=await asyncio.gather(*(tile_job(tx,ty) for tx in range(x0,x1) for ty in range(y0,y1)))
    for item in tiles:
        if item:
            tx,ty,tile=item; base.paste(tile,((tx-x0)*tile_size,(ty-y0)*tile_size))
    params={
        "REQUEST":"GetMap","SERVICE":"WMS","VERSION":"1.1.1","layers":FIRMS_SOURCES,
    ...
    for attempt in range(2):
        try:
            async with session.get(f"{FIRMS_WMS}/{key}/",params=params,headers=headers) as resp:
                raw=await resp.read(); ctype=(resp.headers.get("Content-Type") or "").lower()
                if resp.status != 200: raise RuntimeError(f"FIRMS WMS HTTP {resp.status}: {raw.decode('utf-8',errors='replace')[:300]}")
                if not ctype.startswith("image/") or not raw.startswith(b"\x89PNG"): raise RuntimeError("FIRMS WMS не вернул PNG")
                overlay_data=raw; break
        except Exception as exc:
            last_error=exc
            if attempt==0: await asyncio.sleep(0.7)
    if overlay_data is None: raise last_error
    ...
    font_big=_font(26,True); font_small=_font(16)
    cx,cy=_tile_xy(lon,lat,zoom); px=int(round((cx-x0)*tile_size)); py=int(round((cy-y0)*tile_size))
    draw.ellipse((px-8,py-8,px+8,py+8),fill=(255,255,255,255),outline=(20,30,40,255),width=3)
    draw.text((px+12,py-12),f"  {city}",fill=(20,30,40,255),font=font_small,stroke_width=3,stroke_fill=(255,255,255,220))
    draw.rectangle((0,0,width,52),fill=(15,22,30,225)); draw.text((18,11),f"🔥 Пожары — {city}",fill=(255,255,255,255),font=font_big)
    draw.rectangle((0,height-34,width,height),fill=(15,22,30,225)); draw.text((14,height-27),"NASA FIRMS • VIIRS • последние 24 часа",fill=(235,240,245,255),font=font_small)
    out=io.BytesIO(); base.convert("RGB").save(out,format="PNG",optimize=True); raw=out.getvalue()
