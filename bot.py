...
async def get_weather(lat, lon, hours=48):
    params = {
        ...
        "temperature_unit": "celsius", "wind_speed_unit": "kmh", "precipitation_unit": "mm", "models": "auto"
    }
    async with aiohttp.ClientSession() as session:
...
async def get_profile(lat, lon):
    ...
    params = {"latitude": lat, "longitude": lon, "hourly": hourly, "forecast_hours": 1, "timezone": "auto", "models": "auto"}
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://api.open-meteo.com/v1/forecast", params=params, timeout=20) as resp:
...
async def send_forecast(message, city):
    ...
    try:
        ...
        i=current_index(data); h=data["hourly"]; g=lambda k:h.get(k,[None]*len(h.get("time",[])))[i]
    except Exception as e:
        logging.exception("Forecast error for %s", city)
        await message.answer(f"⚠️ Open-Meteo не вернул прогноз. {html.escape(str(e))}")
        return
...
async def weth_command(message:types.Message):
    # /weth город — прогноз; /weth без города — сохранённый город пользователя.
    city = (message.text or "").partition(" ")[2].strip()
    if not city:
        user = get_user(message.from_user.id)
        city = (user["city"] if user and user["city"] else "").strip()
    if not city:
        await message.answer("Используй: /weth город\nНапример: /weth Москва")
        return
    try:
        await send_forecast(message, city)
    except Exception:
        logging.exception("weth command")
        await message.answer("⚠️ Не удалось получить прогноз. Попробуй ещё раз позже.")
...
async def make_fire_image(city, lat, lon):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        overlay_data = await _download_image(session, wms_url, headers=headers) if False else None
        async with session.get(wms_url, params=params, headers=headers, timeout=35) as resp:
            overlay_data = await resp.read()
            content_type = (resp.headers.get("Content-Type") or "").lower()
            if resp.status != 200:
                raise RuntimeError(f"FIRMS WMS HTTP {resp.status}: {overlay_data[:240]!r}")
            if not overlay_data.startswith(b"\\x89PNG"):
                try:
                    detail = overlay_data.decode("utf-8", "replace")[:500]
                except Exception:
                    detail = repr(overlay_data[:240])
                raise RuntimeError(f"NASA FIRMS вернул не PNG: {detail}")
