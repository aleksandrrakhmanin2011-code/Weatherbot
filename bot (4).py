...
def keyboard(lat,lon,city=None):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🌧 Радар",callback_data=f"radar:{lat:.5f}:{lon:.5f}:{city[:50]}"),
         InlineKeyboardButton(text="🛰 IR Sandwich",callback_data=f"sat:{lat:.5f}:{lon:.5f}:{city[:50]}")]
    ])

def main_menu():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="🌤 Прогноз"),KeyboardButton(text="📈 Графики")],
        [KeyboardButton(text="🌧 Радар"),KeyboardButton(text="🛰 Спутник")],
...
# ---------------- EUMETSAT Meteosat IR Sandwich ----------------
EUMETSAT_WMS = "https://eumetview.eumetsat.int/geoserv/ows"

async def eumetsat_wms_image(layer, bbox, width=900, height=700):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(EUMETSAT_WMS, params=params, timeout=30) as resp:
            resp.raise_for_status()
            raw = await resp.read()
            if not raw.startswith(b"\x89PNG"):
                raise RuntimeError("EUMETSAT WMS вернул не PNG")
            return Image.open(io.BytesIO(raw)).convert("RGBA")

def _sat_colorize(ir):
    # Meteologix/EUMETSAT-like cold cloud-top enhancement: colder -> stronger colors.
    gray = ImageEnhance.Contrast(ir.convert("L")).enhance(1.55)
    px = gray.load(); out = Image.new("RGBA", gray.size); op = out.load()
    for y in range(gray.height):
        for x in range(gray.width):
            v = px[x,y]
            # WMS IR rendering is approximately cold=bright. Map 70..245 to -20..-90-ish.
            t = max(0.0, min(1.0, (v - 90) / 155.0))
            if t < 0.18:
                c = (70, 90, 170)
            elif t < 0.38:
                q = (t-0.18)/0.20; c = (int(70+20*q), int(90+140*q), int(170-30*q))
            elif t < 0.58:
                q = (t-0.38)/0.20; c = (int(90+165*q), int(230-70*q), int(140-120*q))
            elif t < 0.78:
                q = (t-0.58)/0.20; c = (int(255), int(160-110*q), int(20-10*q))
            else:
                q = (t-0.78)/0.22; c = (255, int(50+160*q), int(10+180*q))
            op[x,y] = (*c, int(235*t))
    return out

def make_eumetsat_sandwich(city, lat, lon, vis, ir, bbox):
    # Blend colour-enhanced IR over visible image, reproducing the idea of HRV/VIS + IR10.8 sandwich.
    base = vis.convert("RGBA")
    ir_layer = _sat_colorize(ir)
    img = Image.alpha_composite(base, ir_layer)
    ...
    label = f" {city}"
    draw.rounded_rectangle((x+r+4,y-15,x+r+10+8*len(label),y+14), radius=6, fill=(10,15,22,190))
    draw.text((x+r+8,y-10), label, fill="white")
    draw.rectangle((0,0,w,42), fill=(8,12,18,185))
    draw.text((14,12), "🛰 Meteosat • IR Sandwich (VIS + IR10.8)", fill="white")
    draw.text((14,h-28), "Источник: EUMETSAT / EUMETView", fill="white")
    return img

async def make_satellite_image(city, lat, lon):
    # Regional view around the requested city; wide enough for storm systems.
    half_lon, half_lat = 9.0, 6.0
    bbox = (lat-half_lat, lon-half_lon, lat+half_lat, lon+half_lon)
    try:
        vis, ir = await asyncio.gather(
            eumetsat_wms_image("meteosat:msg_vis006", bbox),
            eumetsat_wms_image("meteosat:msg_ir108", bbox)
        )
        return make_eumetsat_sandwich(city, lat, lon, vis, ir, bbox), "Meteosat SEVIRI / EUMETSAT"
    except Exception:
        logging.exception("EUMETSAT sandwich error")
        # If the visible product is unavailable (for example at night), still return IR.
        ir = await eumetsat_wms_image("meteosat:msg_ir108", bbox)
        draw = ImageDraw.Draw(ir)
        w,h=ir.size; south,west,north,east=bbox
        x=(lon-west)/(east-west)*w; y=(north-lat)/(north-south)*h
        draw.ellipse((x-8,y-8,x+8,y+8),outline="white",width=3); draw.ellipse((x-3,y-3,x+3,y+3),fill="white")
        draw.rectangle((0,0,w,42),fill=(8,12,18,185)); draw.text((14,12),f"🛰 Meteosat • IR10.8 — {city}",fill="white")
        draw.text((14,h-28),"Источник: EUMETSAT / EUMETView",fill="white")
        return ir, "Meteosat SEVIRI / EUMETSAT"

async def send_satellite(message, city):
    lat, lon, resolved, _ = await get_coordinates(city)
    if lat is None:
        await message.answer("😕 Город не найден. Попробуй написать иначе."); return
    try:
        img, source = await make_satellite_image(resolved, lat, lon)
        out=io.BytesIO(); img.save(out, format="PNG"); out.seek(0)
        await message.answer_photo(
            BufferedInputFile(out.read(), filename="eumetsat_ir_sandwich.png"),
            caption=f"🛰 <b>IR Sandwich — {html.escape(resolved)}</b>\n<i>{source}</i>",
            parse_mode="HTML"
        )
    except Exception as e:
        logging.exception("Satellite error")
        await message.answer(f"⚠️ Не удалось загрузить спутниковый снимок. {html.escape(str(e))}", parse_mode="HTML")
...
async def cmd_help(message:types.Message):
    await message.answer("Личные сообщения: кнопки «Прогноз», «Графики», «Радар» и «Спутник».\nГруппы: /wz Самара, /graph Самара, /rad Самара и /satl Самара.")
...
async def radar_button(message:types.Message):
    user=get_user(message.from_user.id)
    if not user or not user["city"]:
        await message.answer("Сначала выбери город в настройках.",reply_markup=main_menu()); return
    await send_radar(message,user["city"])

@dp.message(F.text=="🛰 Спутник",F.chat.type=="private")
async def satellite_button(message:types.Message):
    user=get_user(message.from_user.id)
    if not user or not user["city"]:
        await message.answer("Сначала выбери город в настройках.",reply_markup=main_menu()); return
    await send_satellite(message,user["city"])
...
@dp.callback_query(lambda c:c.data and c.data.startswith("radar:"))
async def radar_callback(callback:types.CallbackQuery):
    await callback.answer("Загружаю радар…")
    try:
        _,lat,lon,city=callback.data.split(":",3)
        await send_radar(callback.message,city)
    except Exception:
        logging.exception("radar callback")
        await callback.message.answer("⚠️ Не удалось загрузить радар.")

@dp.callback_query(lambda c:c.data and c.data.startswith("sat:"))
async def satellite_callback(callback:types.CallbackQuery):
    await callback.answer("Загружаю спутник…")
    try:
        _,lat,lon,city=callback.data.split(":",3)
        await send_satellite(callback.message,city)
    except Exception:
        logging.exception("satellite callback")
        await callback.message.answer("⚠️ Не удалось загрузить спутниковый снимок.")
...
@dp.message(Command("satl"))
async def satellite_command(message:types.Message):
    city=(message.text or "").partition(" ")[2].strip()
    if not city:
        await message.answer("Используй: /satl Самара")
        return
    await send_satellite(message, city)
